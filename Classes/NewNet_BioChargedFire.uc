    ...
    if( class'Freon_BioGlob' != none )
    {
        if(PingDT > 0.0 && Weapon.Owner!=None)
        {
            ...
            for(f=0.00; f<pingDT + timestep; f+=timestep)
            {
                ...
                End = Start + Extrapolate(NewDir, timestep, GoopLoad);
                //Put pawns there
                TimeTravel(pingdt - g);
           ...
           VZ.Z = class'Freon_BioGlob'.default.TossZ;
     //      NewDir =  rotator(vector(NewDir)*class'Freon_BioGlob'.default.speed - VZ);
           if(Other == none)
               glob = Weapon.Spawn(class'Freon_BioGlob',,, End, NewDir);
           else
           {
               glob = Weapon.Spawn(class'Freon_BioGlob',,, HitLocation - Vector(Newdir)*16.0, NewDir);
           }
        }
        else
...
function vector Extrapolate(out rotator Dir, float dF, byte GoopLoad)
{
    ...
    if ( GoopLoad < 1 )
	    GooSpeed =  class'Freon_BioGlob'.default.speed;
	else
	    GooSpeed =  class'Freon_BioGlob'.default.speed * (0.4 + GoopLoad)/(1.4*GoopLoad);
