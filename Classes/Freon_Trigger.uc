...
function Touch(Actor Other)
{
    ...
    if(Other == None || PawnOwner == None  || Other == Owner || !IsRelevant(Other) || Freon_Pawn(Other) == None)
        return;

    touch = Freon_Pawn(Other);
	
    if(!touch.bFrozen && touch.GetTeamNum() == PawnOwner.GetTeamNum()){
...
{
    if(Freon_Pawn(P) != None && Freon_Pawn(P).GetTeamNum() == PawnOwner.GetTeamNum()){
...
function bool TellBotToThaw(Bot B)
{
    ...
    if(B.ActorReachable(PawnOwner) && PawnOwner.GetTeamNum() == B.GetTeamNum())
    {
...
state PawnFrozen
{
    function Touch(Actor Other)
    {
        ...
        if(Other == None || Other == Owner || !IsRelevant(Other) || PawnOwner == None || Freon_Pawn(Other) == None || Team_GameBase(Level.Game).bEndOfRound || Team_GameBase(Level.Game).EndOfRoundTime>0)
            return;

        touch = Freon_Pawn(Other);
		
		if(!touch.bFrozen && PawnOwner != None && touch.GetTeamNum() == PawnOwner.GetTeamNum()){
			Toucher.Length = Toucher.Length + 1;
			Toucher[Toucher.Length - 1] = touch;
...
defaultproperties
{
     ...
     TouchSound=Sound'3SPNv3225PIG.Sounds.Touch'
     ThawSpeed=5.000000
