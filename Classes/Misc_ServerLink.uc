...
function RegisterStats(string GameTime, string PlayerName, string PlayerHash, int TeamIdx, int Rounds, float Score, int Kills, int Deaths, int Thaws, int Git, float HealthGiven)
{
	AddToBuffer("REGISTER_STATS "$GameTime$" "$PlayerName$" "$PlayerHash$" "$TeamIdx$" "$Rounds$" "$Score$" "$Kills$" "$Deaths$" "$Thaws$" "$Git$" "$HealthGiven$LF);
}
