...
struct TrackedData
{
	...
	var int Thaws;
	var int Git;
	var float HealthGiven;
	var float EndOfRoundScore;
};
var config TrackedData Current;

static function AttachPlayerRecord(class<Misc_Util> misc_util_class, Misc_Player P, Misc_PlayerData PD)
{
	local string PlayerName;
	
	//PlayerName = class'Misc_Util'.static.StripColor(P.PlayerReplicationInfo.PlayerName);
	PlayerName = misc_util_class.static.StripColor(P.PlayerReplicationInfo.PlayerName);
	...
	PD.StatsID = misc_util_class.static.GetStatsID(P);
	PD.Owner = P;
	P.PlayerData = PD;
}
