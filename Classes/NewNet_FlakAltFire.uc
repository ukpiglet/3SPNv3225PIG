    ...
    if( ProjectileClass != none )
    {
        if(PingDT > 0.0 && Weapon.Owner!=None)
        {
            ...
            for(f=0.00; f<pingDT + timestep; f+=timestep)
            {
                ...
                End = Start + Extrapolate(NewDir, timestep, g==0.00);
                //Put pawns there
                TimeTravel(pingdt - g);
...
defaultproperties
{
     FakeProjectileClass=Class'3SPNv3225PIG.NewNet_Fake_FlakShell'
     ProjectileClass=Class'3SPNv3225PIG.NewNet_FlakShell'
}
